...
def _build_routes(app_dir: str, cwd: str, config: Any) -> tuple[list[BaseRoute], list[Any]]:
    ...
    for url, directory in (
        ...
        if os.path.isdir(directory):
            routes.append(Mount(url, app=StaticFiles(directory=directory), name=url[1:]))
    routes.extend(page_registry.routes)
    seo = config.seo
    if seo.sitemap_enabled:
        def sitemap(request: Request) -> Response:
            base = seo.base_url or str(request.base_url).rstrip("/")
            return Response(_generate_sitemap_xml(app_dir, base), media_type="application/xml")
...
def _configure_execution_store(config: Any, runtime_engine: Any) -> tuple[Any, str | None]:
    ...
    if provider == "voodoo":
        ...
        store = PostgresExecutionStore(config.database.url or os.getenv("VOODOO_DATABASE_URL", ""))
...
async def _start_mqtt(config: Any, edge_gateway: list[Any], application_store: Any) -> Any:
...
def create_app(app_dir: str = "app", *, runtime: Any = None) -> Starlette:
    ...
    from voodoo.security import CORSMiddleware, CSRFMiddleware, RateLimitMiddleware, SecurityHeadersMiddleware
