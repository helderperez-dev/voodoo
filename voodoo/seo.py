...
class SEO(BaseModel):
    ...
    def render_meta_tags(  # noqa: C901
        ...
        if tw or og_title:
            ...
            if tw_desc:
                tags.append(
                    f'<meta name="twitter:description" content="{_esc(tw_desc)}">'
                )
            if tw_image:
                tags.append(f'<meta name="twitter:image" content="{_esc(tw_image)}">')
            if tw and tw.creator:
                tags.append(
                    f'<meta name="twitter:creator" content="{_esc(tw.creator)}">'
                )
            if tw and tw.site:
        ...
        if geo:
            ...
            if geo.published_date:
                tags.append(
                    f'<meta property="article:published_time" content="{_esc(geo.published_date)}">'
                )
            if geo.modified_date:
                tags.append(
                    f'<meta property="article:modified_time" content="{_esc(geo.modified_date)}">'
                )
