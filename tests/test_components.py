...
def test_button():
    ...
    html = btn.render()
    assert html == '<button id="btn-1" onclick="voodoo.sendEvent(\'my_action\', this.id, this.value)" class="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--color-primary)] disabled:pointer-events-none disabled:opacity-50 bg-[var(--color-text)] text-[var(--color-surface)] hover:bg-[var(--color-text)]/90 h-9 px-4 py-2">Click Me</button>'

def test_input():
    inp = Input(id="inp-1", on_change="input_changed", type="text")
    html = inp.render()
    assert html == '<input id="inp-1" type="text" onchange="voodoo.sendEvent(\'input_changed\', this.id, this.value)" class="flex h-9 w-full rounded-md border border-[var(--color-border)] bg-transparent px-3 py-1 text-sm shadow-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-[var(--color-text-muted)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--color-primary)] disabled:cursor-not-allowed disabled:opacity-50" />'

def test_card():
    card = Card("Card Content", id="card-1", className="extra-class")
    html = card.render()
    assert html == '<div id="card-1" class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-6 shadow-sm extra-class">Card Content</div>'
...
def test_heading():
    ...
    assert h1.render() == '<h1 id="h-1" class="text-4xl font-bold tracking-tight text-[var(--color-text)]">H1 Title</h1>'
    
    h3 = Heading("H3 Title", id="h-3", level=3)
    assert h3.render() == '<h3 id="h-3" class="text-2xl font-semibold tracking-tight text-[var(--color-text)]">H3 Title</h3>'
