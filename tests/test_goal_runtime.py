...
async def test_goal_can_coordinate_multiple_intents():
    runtime, _ = _runtime(
        ComputeParticipant(name="reader", kind="compute", capabilities=["inventory.read"], compute=lambda ctx: {"stock": 2}),
        ComputeParticipant(name="buyer", kind="compute", capabilities=["purchase.create"], compute=lambda ctx: {"purchase_id": "p-1"}),
    )
...
async def test_goal_decomposer_receives_world_snapshot():
    ...
    runtime, _ = _runtime(
        ComputeParticipant(name="buyer", kind="compute", capabilities=["purchase.create"], compute=lambda ctx: "ordered"),
        world=world,
    )
    seen = {}
    ...
    goal = Goal(name="keep-stock", objective="keep widget stock above zero", target_entity_id="warehouse-1")
...
async def test_goal_waits_when_an_intent_needs_human_approval():
    runtime, engine = _runtime(
        ComputeParticipant(name="payer", kind="compute", capabilities=["payment.execute"], compute=lambda ctx: "paid")
    )
    engine.capabilities.require_approval("payment.execute")
    goal = Goal(name="settle-invoice")
    run = await runtime.achieve(goal, intents=[Intent(name="pay").require("payment.execute")])
...
async def test_goal_stops_after_failed_intent():
    ...
    runtime, _ = _runtime(
        ComputeParticipant(name="supplier", kind="compute", capabilities=["supplier.quote"], compute=fail),
        ComputeParticipant(name="buyer", kind="compute", capabilities=["purchase.create"], compute=should_not_run),
    )
    goal = Goal(name="replenish")
    run = await runtime.achieve(goal, intents=[Intent(name="quote").require("supplier.quote"), Intent(name="buy").require("purchase.create")])
...
async def test_goal_can_generate_single_intent_from_required_capabilities():
    runtime, _ = _runtime(
        ComputeParticipant(name="inspector", kind="compute", capabilities=["machine.inspect"], compute=lambda ctx: "healthy")
    )
    goal = Goal(name="verify-machine", objective="ensure the machine is healthy", requires=["machine.inspect"])
