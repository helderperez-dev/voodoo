...
import json
import pytest
from voodoo.seo import SEO, OpenGraph, TwitterCard, GEO, FAQ, _esc
from voodoo.components import (
    Component, Div, Heading, Nav, Header, Footer, Main, Section,
    Article, Aside, Figure, FigCaption, Time, Address, Img, Paragraph
)
...
class TestTwitterCard:
    def test_twitter_explicit(self):
        seo = SEO(
            twitter=TwitterCard(title="TW Title", description="TW Desc", creator="@user")
        )
        tags = seo.render_meta_tags()
...
class TestStructuredData:
    ...
    def test_auto_article_schema_from_geo(self):
        ...
        parsed = json.loads(ld.split('<script type="application/ld+json">')[1].split('</script>')[0])
    ...
    def test_auto_faq_schema_from_geo(self):
        ...
        assert parsed["mainEntity"][0]["acceptedAnswer"]["text"] == "A Python framework."
    
    def test_no_duplicate_article_schema(self):
        """If user provides Article schema, GEO should not auto-generate another."""
        custom_article = {"@context": "https://schema.org", "@type": "Article", "headline": "Custom"}
        seo = SEO(
    ...
    def test_no_duplicate_faq_schema(self):
        ...
        custom_faq = {"@context": "https://schema.org", "@type": "FAQPage", "mainEntity": []}
        seo = SEO(
            structured_data=[custom_faq],
            geo=GEO(faq=[FAQ(question="Q?", answer="A.")])
        )
        ld = seo.render_structured_data()
...
class TestRenderPage:
    ...
    def test_structured_data_in_page(self):
        seo = SEO(
            structured_data=[{"@context": "https://schema.org", "@type": "WebSite", "name": "Test"}]
        )
        html = render_page(Div("x", id="t"), seo=seo)
